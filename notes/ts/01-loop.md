...
事件总线、JSONL、AbortSignal、`read`/`bash`、Responses API。缺了它们 loop 仍然能跑完一个 turn。原始 pi-agent 也不读 `AGENTS.md`；本仓库根目录 [`AGENTS.md`](../../AGENTS.md) 只约束我们怎么写代码，不进模型上下文。
...
第 2 片：[02-events.md](02-events.md)。同一套 loop 的 Go + Gin 对照：[../go/01-loop.md](../go/01-loop.md)。
